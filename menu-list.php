            ...
            if (isset($row['6']) && $row['6'] != '') {
              ...
              <tr class='my_text'>
                <td class="primary_menu_seprator"><span class="dashicons-before  <?php echo esc_attr($row['6']); ?>"></span>
                  <span>
                    <?php echo esc_attr(strip_tags($row['0'])); ?>
                  </span>
                </td>
                <?php foreach ($roles as $role_key => $role) { ?>
                  <td class="primary_menu_seprator">
                    <input <?php if (in_array($row['2'], isset($fetch_hide_menu_array->$role_key) ? $fetch_hide_menu_array->$role_key : array()))
                      echo 'checked'; ?> type="checkbox"
                      name="menu_list[<?php echo $role_key ?>][]" value="<?php echo esc_attr($key); ?>">
                  </td>
                <?php } ?>
              </tr>
              ...
              foreach ($sub_menu_array as $keys => $rows) {
                ...
                <tr class='my_text sub-menu'>
                  ...
                  <td><span>-- </span>
                    <span>
                      <?php echo esc_attr(strip_tags($rows['0'])); ?>
                    </span>
                  </td>
                  <?php foreach ($roles as $role_key => $role) { ?>
                    <td>
                      <input <?php if (in_array($rows['2'], isset($fetch_hide_sub_menu_array[$role_key]) ? $fetch_hide_sub_menu_array[$role_key] : array()))
                        echo 'checked'; ?> type="checkbox"
                        name="sub_menu_list[<?php echo $role_key ?>][]"
                        value="<?php echo $key; ?>_<?php echo esc_attr($keys); ?>">
                    </td>
                  <?php } ?>
          ...
          foreach ($all_parent_menu as $row) {
            ...
            if ($row->title != '') {
              ...
              <tr class='my_text'>
                ...
                <?php foreach ($roles as $role_key => $role) { ?>
                  <td>
                    <input <?php if (in_array($row->id, isset($fetch_hide_top_menu_array->$role_key) ? $fetch_hide_top_menu_array->$role_key : array()))
                      echo 'checked'; ?> type="checkbox"
                      name="top_menu_list[<?php echo $role_key ?>][]" value="<?php echo esc_attr($row->id); ?>">
                  </td>
                <?php } ?>
              </tr>
              ...
              if (isset($row->child_menu)) {
                foreach ($row->child_menu as $child_menu) {
                  if ($child_menu->title != '') {
                    ...
                    <?php foreach ($roles as $role_key => $role) { ?>
                      <td>
                        <input <?php if (in_array($child_menu->id, isset($fetch_hide_top_menu_array->role_key) ? $fetch_hide_top_menu_array->role_key : array()))
                          echo 'checked'; ?> type="checkbox"
                          name="top_menu_list[<?php echo $role_key ?>][]" value="<?php echo esc_attr($child_menu->id); ?>">
                      </td>
                    <?php } ?>
                    </tr>
                    <?php
