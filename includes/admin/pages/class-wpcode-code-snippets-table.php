...
class WPCode_Code_Snippets_Table extends WP_List_Table
{
	...
	protected function extra_tablenav($which)
	{
		if ('top' === $which && 'trash' !== $this->view) {
			...
			submit_button(__('Lọc', 'insert-headers-and-footers'), '', 'filter_action', false, array('id' => 'wpcode-filter-submit'));

			if (isset($_GET['filter_action']) || isset($_GET['tag'])) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended
				echo '&nbsp;';
				submit_button(__('Xóa lọc', 'insert-headers-and-footers'), '', 'filter_clear', false, array('id' => 'wpcode-filter-clear'));
			}
		}
	}
...
<label for="filter-by-type" class="screen-reader-text">
    <?php esc_html_e('Filter by code type', 'insert-headers-and-footers'); ?>
</label>
<select name="type" id="filter-by-type">
    <option<?php selected($displayed_type, ''); ?> value="">
        <?php esc_html_e('Tất cả các kiểu', 'insert-headers-and-footers'); ?>
        </option>
        <?php
				foreach ($used_types as $used_type) {
					...
					?>
        <option<?php selected($displayed_type, $used_type->slug); ?> value="<?php echo esc_attr($used_type->slug); ?>">
            <?php echo esc_html($pretty_name); ?>
            </option>
            <?php
				}
				?>
...
<?php
	...
	protected function location_dropdown($post_type)
	{
		...
		?>
<label for="filter-by-location" class="screen-reader-text">
    <?php esc_html_e('Filter by location', 'insert-headers-and-footers'); ?>
</label>
<select name="location" id="filter-by-location">
    <option<?php selected($displayed_location, ''); ?> value="">
        <?php esc_html_e('Tất cả vị trí', 'insert-headers-and-footers'); ?>
        </option>
        <?php
				foreach ($used_locations as $used_location) {
					$pretty_name = wpcode()->auto_insert->get_location_label($used_location->slug);
					?>
        <option<?php selected($displayed_location, $used_location->term_id); ?>
            value="<?php echo esc_attr($used_location->term_id); ?>">
            <?php echo esc_html($pretty_name); ?>
            </option>
            <?php
				}
